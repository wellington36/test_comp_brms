functions{
  ...
  real brms_log_Z_com_poisson(real log_mu, real nu) {
    real log_Z;
    int k = 2;
    int M = 10000;
    int converged = 0;
  ...
  //   log_mu: log location parameter
  //   shape: positive shape parameter
  real brms_com_poisson_log_lpmf(int y, real log_mu, real nu) {
    if (nu == 1) return poisson_log_lpmf(y | log_mu);
    return nu * (y * log_mu - lgamma(y + 1)) - brms_log_Z_com_poisson(log_mu, nu);
  }
  // COM Poisson log-PMF for a single response
  real brms_com_poisson_lpmf(int y, real mu, real nu) {
    if (nu == 1) return poisson_lpmf(y | mu);
    return brms_com_poisson_log_lpmf(y | log(mu), nu);
  }
  // COM Poisson log-CDF for a single response
  real brms_com_poisson_lcdf(int y, real mu, real nu) {
    real log_mu;
    ...
    log_Z = brms_log_Z_com_poisson(log_mu, nu);
    if (y == 0) {
      return -log_Z;
    }
    // first 2 terms of the series
  ...
  real brms_com_poisson_lccdf(int y, real mu, real nu) {
    return log1m_exp(brms_com_poisson_lcdf(y | mu, nu));
  }
}